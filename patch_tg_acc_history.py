...
"""
content = content.replace("const [users, setUsers] = useState<any[]>([]);", state_injection)
...
"""
content = content.replace("const handleSavePwd = async (id: number) => {", fetch_func + "\n  const handleSavePwd = async (id: number) => {")
...
"""
if "className=\"p-2 text-red-600" in content:
    content = content.replace("className=\"p-2 text-red-600", button_html + "\n                      <button className=\"p-2 text-red-600")
